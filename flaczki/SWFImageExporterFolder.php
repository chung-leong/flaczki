...
class SWFImageExporterFolder extends SWFImageExporter {
	...
	public function export($images) {
		foreach($images as $image) {
			if($image->alphaData && $this->canUseGD) {
				$path = "{$this->folderPath}/{$image->name}.png";
				$this->saveAsTransparentPNG($path, $image->imageData, $image->alphaData);
			} else {
				...
				$path = "{$this->folderPath}/{$image->name}{$extension}";
				file_put_contents($path, $image->imageData);
			}
		}
	}
	
	protected function saveAsTransparentPNG($path, $imageData, $compressedAlphaData) {
		$alphaData = gzuncompress($compressedAlphaData);
		$image = imagecreatefromstring($imageData);

		// get the GD raw data, using output buffering as
		...
		// capture the PNG data and save it
		imagesavealpha($image, true);		
		ob_start();
		imagepng($image);
		imagedestroy($image);
