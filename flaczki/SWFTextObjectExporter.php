...
abstract class SWFTextObjectExporter {
	...
	public function export($textObjects, $fontFamilies) {
		...
		foreach($textObjects as $textObject) {
			if(!$this->ignoreAutogenerated || !preg_match('/^__id\d+_$/', $textObject->name)) {
				...
				if(!$this->ignorePointText || $tlfObject->type != 'Point') {
					$section = new SWFTextObjectExportSection;
					$section->name = $textObject->name;
					$section->tlfObject = $tlfObject;
					$sections[] = $section;
				}
			}
		}
	...
	public function setPolicy($policy, $value) {
		switch($policy) {
			case IGNORE_AUTOGENERATED: $this->ignoreAutogenerated = $value; break;
			case IGNORE_POINT_TEXT: $this->ignorePointText = $value; break;
		}
	}
